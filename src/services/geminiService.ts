...
export async function searchCaseInfo(query: string, caseType?: string, jurisdiction?: string, individualDetails?: IndividualDetails) {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      If searching for an individual, look for pending cases, active warrants, past judgments, or any other relevant legal information from official state, county, and federal databases.
      ...
      Provide:
      1. A summary of the case(s) or legal records found and their current status.
...
export async function analyzeCaseDocument(base64Data: string, mimeType: string) {
  try {
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: [
